...
final class HotkeyManager: @unchecked Sendable {
    ...
    private func installFnHoldMonitor() {
        let handler: (NSEvent) -> Void = { [weak self] event in
            guard let self, !self.isToggleActive else { return }
            let now = ProcessInfo.processInfo.systemUptime
            guard now - self.lastEventTime > self.debounceInterval else { return }
        ...
        let global = NSEvent.addGlobalMonitorForEvents(matching: .flagsChanged, handler: handler)
